...
export matlab_path=$basedir/scenario_configs/metis_alone_utils
...
files=(
  "metis_interpretative_from_dina.ymmsl"
)
for file in "${files[@]}"; do
  if test -f ".$file"; then
    cp ".$file" $file
  else
    cp "$basedir/scenario_configs/metis_alone_utils/.$file" $file
  fi
...
RUNDIR="tmp/m3_runs/run_metis-$(date +%F)-$(date +%H%M%S)" 
mkdir -p $RUNDIR
muscle_manager --start-all metis_interpretative_from_dina.ymmsl --run-dir $RUNDIR

echo "$(date +%H):$(date +%M):$(date +%S) PLOTTING"
python $basedir/scenario_configs/metis_alone_utils/plot_validation_metis.py \
  --shot_nr $SHOT_NR \
  --dina_uri "$PWD/tmp/data/${SHOT_NR}_dina_update_in" \
