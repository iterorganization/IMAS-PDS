...
#  --source_uri "imas:hdf5?path=/work/imas/shared/imasdb/ITER/3/105092/1" \
#  --backup_uri "imas:hdf5?path=/home/ITER/sanderm/public/imasdb/ITER/4/666666/3" \
#  --sink_uri "imas:hdf5?path=$(pwd)/tmp/data/105084_in" \
#  --n_timeslices 51
imas convert "imas:mdsplus?path=/home/ITER/dubrovm/public/imasdb/iter/3/105092/1" $IMAS_VERSION "imas:hdf5?path=$(pwd)/tmp/data/105092_in"

# make METIS dataset
echo "$(date +%H):$(date +%M):$(date +%S) Making METIS dataset"
matlab -batch "[s,t] = unix('which python');pyenv('Version',strtrim(t),'ExecutionMode','InProcess'); addpath(pwd);cd('tmp');make_metis_from_dina_105092_interpretative;"

echo "$(date +%H):$(date +%M):$(date +%S) CREATING RUNNABLE YMMSL FILE"
  # Use sed to replace the matching substrings
  file="metis_from_dina_interpretative_105092.ymmsl"
...
mkdir -p $RUNDIR
muscle_manager --start-all metis_from_dina_interpretative_105092.ymmsl --run-dir $RUNDIR
