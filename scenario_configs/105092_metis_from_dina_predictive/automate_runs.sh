...
matlab -batch "[s,t] = unix('which python');pyenv('Version',strtrim(t),'ExecutionMode','InProcess'); addpath(pwd);cd('tmp');make_metis_from_dina_105092_predictive;"

echo "$(date +%H):$(date +%M):$(date +%S) CREATING RUNNABLE YMMSL FILE"
  # Use sed to replace the matching substrings
  file="metis_from_dina_predictive_105092.ymmsl"
...
mkdir -p $RUNDIR
muscle_manager --start-all metis_from_dina_predictive_105092.ymmsl --run-dir $RUNDIR
