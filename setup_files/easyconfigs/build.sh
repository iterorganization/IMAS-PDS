...
declare -A UPSTREAM_PR=(
  ...
  [IMAS-Validator]=26550    # 1.0.0 -- but see the local override, below
  [Waveform-Editor]=26553   # 0.3.1
)
UPSTREAM_ORDER=(IMAS-Cpp IMAS-Python MUSCLE3 INTERPOS iWrap IMAS-Validator Waveform-Editor)
...
declare -A LOCAL_EC=(
  ...
  [NICE]=n/NICE/NICE-3.0.0-intel-2025b-pds.eb
  [CHEASE]=c/CHEASE/CHEASE-2026.08-intel-2025b-pds.eb
  [TORAX-MUSCLE3]=t/TORAX-MUSCLE3/TORAX-MUSCLE3-0.1.2-intel-2025b-pds.eb
...
LOCAL_ORDER=(IMAS-MUSCLE3 NICE CHEASE TORAX-MUSCLE3 METIS-IRFM PCS ymmsl2svg PDS)
...
for name in "${UPSTREAM_ORDER[@]}"; do
  ...
  # IMAS-Validator is the one upstream PR whose easyconfig we override locally;
  # --robot puts this directory first, so the patched file is the one used.
