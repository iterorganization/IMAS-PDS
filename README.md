...
once into a shared `PDS-<Name>` module by the scripts in `setup_files/custom_modules/`
(see `setup_files/PDS.lua` for the full list). Loading the PDS meta-module is enough to
use any of them -- there is nothing to install per-checkout:
...
```bash
# load the PDS module stack (see setup_files/PDS.lua) -- run_workflow.sh
# requires this to already be loaded, from this checkout's root
module use /home/ITER/blokhus/public/modules  # or wherever PDS.lua was deployed
module load PDS
