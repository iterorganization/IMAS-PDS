...
# from this shot's raw DINA source. Identical dual build to
# metis_nice_inverse_from_dina/preprocess.sh (same two DINA-derived datasets are needed here);
# ported from itergit/feature/metis_nice_evol's
# workflows/metis_predictive_nice_evol_from_dina/preprocess_data.sh. Run once by
...
#
# source_metis.source_uri / synchro_nice_metis.source_uri -> ${CASE_DIR}/preprocess/metis_in
...
# $CASE_DIR/preprocess_settings.ymmsl, stacked in by bin/pds-run-case.sbatch -- matters here
# because NICE's evolutive free-boundary solve carries psi_LCFS forward from METIS.
set -euo pipefail
